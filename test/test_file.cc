...
signed main() {
  ...
  if constexpr (false) {
    try {
      std::string s;
      system_control::Status::refresh("try function between num and string");
      system_control::Status::print();
      [&](bool __skip) {
        if (__skip) return;
        system_control::Status::announce("int");
        int alpha;
        IO(alpha);
      ...
      [&](bool __skip) {
        if (__skip) return;
        system_control::Status::announce("unsigned int");
        unsigned int alpha;
        IO(alpha);
        file_io::numToString(s, alpha);
        SHOW(s);
      ...
      [&](bool __skip) {
        if (__skip) return;
        system_control::Status::announce("long long");
        long long alpha;
        IO(alpha);
        file_io::numToString(s, alpha);
        SHOW(s);
      ...
      [&](bool __skip) {
        if (__skip) return;
        system_control::Status::announce("unsigned long long");
  ...
  if constexpr (false) {
    system_control::Status::refresh("try rom io");
    system_control::Status::print();
    file_io::FileIO file;
    try {
      [&] {
        system_control::Status::announce("<set file> function of RomIO");
    ...
    try {
      [&] {
        system_control::Status::announce("<writing file> function of RomIO 01");
    ...
    try {
      [&] {
        system_control::Status::announce("<writing file> function of RomIO 02");
        file.switchMode(file_io::FileIO::writing);
        file.print();
    ...
    try {
      [&] {
        system_control::Status::announce("<reading file> function of RomIO");
  ...
  if constexpr (true) {
    system_control::Status::refresh("File IO Ordered");
    system_control::Status::print();
    file_io::FileIOOrdered orderFile;
    orderFile.setFile(__FILE__, "order");
    orderFile.print();
    [&] {
      system_control::Status::announce("write file");
      auto alpha = basic_math::uniformRand(8, -5, 5);
      SHOW(alpha);
    ...
    [&] {
      system_control::Status::announce("read file");
