...
template <class T> constexpr bool check_simd = is_simd_on<T>::value;
...
return the answer*/
template <typename Data>
static inline __attribute__((__always_inline__)) Data random(Data const &alpha,
                                                             Data const &beta) {
...
#endif
    long long range =
        static_cast<long long>(beta) - static_cast<long long>(alpha) + 1;
...
static inline __attribute__((__always_inline__)) int
size_check(int const &alpha) {
  if (alpha <= 0)
    return vec_len;
  int gamma = alpha / vec_len;
  gamma += (alpha % vec_len) ? 1 : 0;
  gamma *= vec_len;
  return gamma;
}
/*below are the functions you don't need to use*/
...
tuple_add(Data *const &alpha, Data *const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_add_ps(_mm_load_ps(alpha), _mm_load_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_add_ps(_mm256_load_ps(alpha), _mm256_load_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_add_s_(Data *const &alpha, Data const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_add_ps(_mm_load_ps(alpha), _mm_set1_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_add_ps(_mm256_load_ps(alpha), _mm256_set1_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = alpha[0] || beta;
    gamma[1] = alpha[1] || beta;
    gamma[2] = alpha[2] || beta;
    gamma[3] = alpha[3] || beta;
#elif defined(_SIMD_02_)
    gamma[0] = alpha[0] || beta;
    gamma[1] = alpha[1] || beta;
    gamma[2] = alpha[2] || beta;
    gamma[3] = alpha[3] || beta;
    gamma[4] = alpha[4] || beta;
    gamma[5] = alpha[5] || beta;
    gamma[6] = alpha[6] || beta;
    gamma[7] = alpha[7] || beta;
#else
    gamma[0] = alpha[0] || beta;
    gamma[1] = alpha[1] || beta;
    gamma[2] = alpha[2] || beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sub(Data *const &alpha, Data *const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_sub_ps(_mm_load_ps(alpha), _mm_load_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_sub_ps(_mm256_load_ps(alpha), _mm256_load_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = alpha[0] || (!beta[0]);
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sub_sb_(Data *const &alpha, Data const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_sub_ps(_mm_load_ps(alpha), _mm_set1_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_sub_ps(_mm256_load_ps(alpha), _mm256_set1_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = alpha[0] || (!beta);
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sub_sf_(Data const &alpha, Data *const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_sub_ps(_mm_set1_ps(alpha), _mm_load_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_sub_ps(_mm256_set1_ps(alpha), _mm256_load_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = alpha || (!beta[0]);
    gamma[1] = alpha || (!beta[1]);
    gamma[2] = alpha || (!beta[2]);
    gamma[3] = alpha || (!beta[3]);
#elif defined(_SIMD_02_)
    gamma[0] = alpha || (!beta[0]);
    gamma[1] = alpha || (!beta[1]);
    gamma[2] = alpha || (!beta[2]);
    gamma[3] = alpha || (!beta[3]);
    gamma[4] = alpha || (!beta[4]);
    gamma[5] = alpha || (!beta[5]);
    gamma[6] = alpha || (!beta[6]);
    gamma[7] = alpha || (!beta[7]);
#else
    gamma[0] = alpha || (!beta[0]);
    gamma[1] = alpha || (!beta[1]);
    gamma[2] = alpha || (!beta[2]);
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_mul(Data *const &alpha, Data *const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_mul_ps(_mm_load_ps(alpha), _mm_load_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_mul_ps(_mm256_load_ps(alpha), _mm256_load_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = alpha[0] && beta[0];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_mul_s_(Data *const &alpha, Data const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_mul_ps(_mm_load_ps(alpha), _mm_set1_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_mul_ps(_mm256_load_ps(alpha), _mm256_set1_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = alpha[0] && beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_div(Data *const &alpha, Data *const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_div_ps(_mm_load_ps(alpha), _mm_load_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_div_ps(_mm256_load_ps(alpha), _mm256_load_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = (alpha[0] || beta[0]) && (!(alpha[0] && beta[0]));
    gamma[1] = (alpha[1] || beta[1]) && (!(alpha[1] && beta[1]));
    gamma[2] = (alpha[2] || beta[2]) && (!(alpha[2] && beta[2]));
    gamma[3] = (alpha[3] || beta[3]) && (!(alpha[3] && beta[3]));
#elif defined(_SIMD_02_)
    gamma[0] = (alpha[0] || beta[0]) && (!(alpha[0] && beta[0]));
    gamma[1] = (alpha[1] || beta[1]) && (!(alpha[1] && beta[1]));
    gamma[2] = (alpha[2] || beta[2]) && (!(alpha[2] && beta[2]));
    gamma[3] = (alpha[3] || beta[3]) && (!(alpha[3] && beta[3]));
    gamma[4] = (alpha[4] || beta[4]) && (!(alpha[4] && beta[4]));
    gamma[5] = (alpha[5] || beta[5]) && (!(alpha[5] && beta[5]));
    gamma[6] = (alpha[6] || beta[6]) && (!(alpha[6] && beta[6]));
    gamma[7] = (alpha[7] || beta[7]) && (!(alpha[7] && beta[7]));
#else
    gamma[0] = (alpha[0] || beta[0]) && (!(alpha[0] && beta[0]));
    gamma[1] = (alpha[1] || beta[1]) && (!(alpha[1] && beta[1]));
    gamma[2] = (alpha[2] || beta[2]) && (!(alpha[2] && beta[2]));
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_div_sb_(Data *const &alpha, Data const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_div_ps(_mm_load_ps(alpha), _mm_set1_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_div_ps(_mm256_load_ps(alpha), _mm256_set1_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = (alpha[0] || beta) && (!(alpha[0] && beta));
    gamma[1] = (alpha[1] || beta) && (!(alpha[1] && beta));
    gamma[2] = (alpha[2] || beta) && (!(alpha[2] && beta));
    gamma[3] = (alpha[3] || beta) && (!(alpha[3] && beta));
#elif defined(_SIMD_02_)
    gamma[0] = (alpha[0] || beta) && (!(alpha[0] && beta));
    gamma[1] = (alpha[1] || beta) && (!(alpha[1] && beta));
    gamma[2] = (alpha[2] || beta) && (!(alpha[2] && beta));
    gamma[3] = (alpha[3] || beta) && (!(alpha[3] && beta));
    gamma[4] = (alpha[4] || beta) && (!(alpha[4] && beta));
    gamma[5] = (alpha[5] || beta) && (!(alpha[5] && beta));
    gamma[6] = (alpha[6] || beta) && (!(alpha[6] && beta));
    gamma[7] = (alpha[7] || beta) && (!(alpha[7] && beta));
#else
    gamma[0] = (alpha[0] || beta) && (!(alpha[0] && beta));
    gamma[1] = (alpha[1] || beta) && (!(alpha[1] && beta));
    gamma[2] = (alpha[2] || beta) && (!(alpha[2] && beta));
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_div_sf_(Data const &alpha, Data *const &beta, Data *const &gamma) {
  if constexpr ((std::is_same_v<Data, float>) && (SIMD_ON)) {
#if defined(_SIMD_01_)
    _mm_store_ps(gamma, _mm_div_ps(_mm_set1_ps(alpha), _mm_load_ps(beta)));
#elif defined(_SIMD_02_)
    _mm256_store_ps(gamma,
                    _mm256_div_ps(_mm256_set1_ps(alpha), _mm256_load_ps(beta)));
#endif
  } else if constexpr (std::is_same_v<Data, bool>) {
#if defined(_SIMD_01_)
    gamma[0] = (alpha || beta[0]) && (!(alpha && beta[0]));
    gamma[1] = (alpha || beta[1]) && (!(alpha && beta[1]));
    gamma[2] = (alpha || beta[2]) && (!(alpha && beta[2]));
    gamma[3] = (alpha || beta[3]) && (!(alpha && beta[3]));
#elif defined(_SIMD_02_)
    gamma[0] = (alpha || beta[0]) && (!(alpha && beta[0]));
    gamma[1] = (alpha || beta[1]) && (!(alpha && beta[1]));
    gamma[2] = (alpha || beta[2]) && (!(alpha && beta[2]));
    gamma[3] = (alpha || beta[3]) && (!(alpha && beta[3]));
    gamma[4] = (alpha || beta[4]) && (!(alpha && beta[4]));
    gamma[5] = (alpha || beta[5]) && (!(alpha && beta[5]));
    gamma[6] = (alpha || beta[6]) && (!(alpha && beta[6]));
    gamma[7] = (alpha || beta[7]) && (!(alpha && beta[7]));
#else
    gamma[0] = (alpha || beta[0]) && (!(alpha && beta[0]));
    gamma[1] = (alpha || beta[1]) && (!(alpha && beta[1]));
    gamma[2] = (alpha || beta[2]) && (!(alpha && beta[2]));
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_eq_v(Data *const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] == beta[0];
  gamma[1] = alpha[1] == beta[1];
  gamma[2] = alpha[2] == beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_eq_s(Data *const &alpha, Data const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] == beta;
  gamma[1] = alpha[1] == beta;
  gamma[2] = alpha[2] == beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_ne_v(Data *const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] != beta[0];
  gamma[1] = alpha[1] != beta[1];
  gamma[2] = alpha[2] != beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_ne_s(Data *const &alpha, Data const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] != beta;
  gamma[1] = alpha[1] != beta;
  gamma[2] = alpha[2] != beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_bg_v(Data *const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] > beta[0];
  gamma[1] = alpha[1] > beta[1];
  gamma[2] = alpha[2] > beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_bg_sb(Data *const &alpha, Data const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] > beta;
  gamma[1] = alpha[1] > beta;
  gamma[2] = alpha[2] > beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_bg_sf(Data const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha > beta[0];
  gamma[1] = alpha > beta[1];
  gamma[2] = alpha > beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_bq_v(Data *const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] >= beta[0];
  gamma[1] = alpha[1] >= beta[1];
  gamma[2] = alpha[2] >= beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_bq_sb(Data *const &alpha, Data const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] >= beta;
  gamma[1] = alpha[1] >= beta;
  gamma[2] = alpha[2] >= beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_bq_sf(Data const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha >= beta[0];
  gamma[1] = alpha >= beta[1];
  gamma[2] = alpha >= beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sm_v(Data *const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] < beta[0];
  gamma[1] = alpha[1] < beta[1];
  gamma[2] = alpha[2] < beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sm_sb(Data *const &alpha, Data const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] < beta;
  gamma[1] = alpha[1] < beta;
  gamma[2] = alpha[2] < beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sm_sf(Data const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha < beta[0];
  gamma[1] = alpha < beta[1];
  gamma[2] = alpha < beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sq_v(Data *const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] <= beta[0];
  gamma[1] = alpha[1] <= beta[1];
  gamma[2] = alpha[2] <= beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sq_sb(Data *const &alpha, Data const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha[0] <= beta;
  gamma[1] = alpha[1] <= beta;
  gamma[2] = alpha[2] <= beta;
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_sq_sf(Data const &alpha, Data *const &beta, bool *const &gamma) {
#if defined(_SIMD_01_)
  gamma[0] = alpha <= beta[0];
  gamma[1] = alpha <= beta[1];
  gamma[2] = alpha <= beta[2];
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_abs(Data *const &alpha, Data *const &gamma) {
#ifdef _DEBUG_MODE_
  printf("~tuple abs at %p~\n", alpha);
#endif
  if constexpr (std::is_same_v<Data, float>) {
#if defined(_SIMD_01_)
    gamma[0] = std::fabs(alpha[0]);
    gamma[1] = std::fabs(alpha[1]);
    gamma[2] = std::fabs(alpha[2]);
    gamma[3] = std::fabs(alpha[3]);
#elif defined(_SIMD_02_)
    gamma[0] = std::fabs(alpha[0]);
    gamma[1] = std::fabs(alpha[1]);
    gamma[2] = std::fabs(alpha[2]);
    gamma[3] = std::fabs(alpha[3]);
    gamma[4] = std::fabs(alpha[4]);
    gamma[5] = std::fabs(alpha[5]);
    gamma[6] = std::fabs(alpha[6]);
    gamma[7] = std::fabs(alpha[7]);
#else
    gamma[0] = std::fabs(alpha[0]);
    gamma[1] = std::fabs(alpha[1]);
    gamma[2] = std::fabs(alpha[2]);
...
#if defined(_SIMD_01_)
    gamma[0] = std::abs(alpha[0]);
    gamma[1] = std::abs(alpha[1]);
    gamma[2] = std::abs(alpha[2]);
    gamma[3] = std::abs(alpha[3]);
#elif defined(_SIMD_02_)
    gamma[0] = std::abs(alpha[0]);
    gamma[1] = std::abs(alpha[1]);
    gamma[2] = std::abs(alpha[2]);
    gamma[3] = std::abs(alpha[3]);
    gamma[4] = std::abs(alpha[4]);
    gamma[5] = std::abs(alpha[5]);
    gamma[6] = std::abs(alpha[6]);
    gamma[7] = std::abs(alpha[7]);
#else
    gamma[0] = std::abs(alpha[0]);
    gamma[1] = std::abs(alpha[1]);
    gamma[2] = std::abs(alpha[2]);
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_rand(Data *const &gamma, Data const &alpha, Data const &beta) {
#if defined(_SIMD_01_)
  gamma[0] = Basic_Math::random(alpha, beta);
  gamma[1] = Basic_Math::random(alpha, beta);
  gamma[2] = Basic_Math::random(alpha, beta);
  gamma[3] = Basic_Math::random(alpha, beta);
#elif defined(_SIMD_02_)
  gamma[0] = Basic_Math::random(alpha, beta);
  gamma[1] = Basic_Math::random(alpha, beta);
  gamma[2] = Basic_Math::random(alpha, beta);
  gamma[3] = Basic_Math::random(alpha, beta);
  gamma[4] = Basic_Math::random(alpha, beta);
  gamma[5] = Basic_Math::random(alpha, beta);
  gamma[6] = Basic_Math::random(alpha, beta);
  gamma[7] = Basic_Math::random(alpha, beta);
#else
  gamma[0] = Basic_Math::random(alpha, beta);
  gamma[1] = Basic_Math::random(alpha, beta);
  gamma[2] = Basic_Math::random(alpha, beta);
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_set(Data *const &gamma, Data const &alpha) {
#ifdef _DEBUG_MODE_
...
template <typename Data>
static inline __attribute__((__always_inline__)) void
tuple_load(Data *const &alpha, Data *const &gamma) {
#ifdef _DEBUG_MODE_
...
check_legal(MaShape const &alpha) {
  return alpha.rows > 0 && alpha.lines > 0;
}
...
// a function which can and a Vector into a Matrix
template <typename Data> void AddRow_(Matrix<Data> &, Vector<Data> const &);
} // namespace Linalg
#define Ln Linalg
...
_mmy_order(_mmy_data &alpha, Data const &beta) {
  if constexpr (std::is_same_v<Data, Ln::Vector<int> *>) {
    alpha.type = Vi;
    alpha.ptr.vi = beta;
    return true;
  } else if constexpr (std::is_same_v<Data, Ln::Vector<bool> *>) {
    alpha.type = Vb;
    alpha.ptr.vb = beta;
    return true;
  } else if constexpr (std::is_same_v<Data, Ln::Vector<float> *>) {
    alpha.type = Vf;
    alpha.ptr.vf = beta;
    return true;
  } else if constexpr (std::is_same_v<Data, Ln::Matrix<int> *>) {
    alpha.type = Mi;
    alpha.ptr.mi = beta;
    return true;
  } else if constexpr (std::is_same_v<Data, Ln::Matrix<bool> *>) {
    alpha.type = Mb;
    alpha.ptr.mb = beta;
    return true;
...
/*below is the function you don't need to use*/
template <typename Data>
static inline __attribute__((__unused__)) bool
_mmy_catch(_mmy_data const &alpha, Data &beta) {
  if (alpha.type == Vi) {
    beta = alpha.ptr.vi;
    return true;
  } else if (alpha.type == Vb) {
    beta = alpha.ptr.vb;
    return true;
  } else if (alpha.type == Vf) {
    beta = alpha.ptr.vb;
    return true;
  } else if (alpha.type == Mi) {
    beta = alpha.ptr.mi;
    return true;
  } else if (alpha.type == Mb) {
    beta = alpha.ptr.mb;
    return true;
...
template <typename Data>
static inline __attribute__((__always_inline__)) bool
_mmy_cmp(_mmy_data const &alpha, Data const &beta) {
  if constexpr (std::is_same_v<Data, Ln::Vector<int> *>) {
    return (alpha.ptr.vi == beta);
  } else if constexpr (std::is_same_v<Data, Ln::Vector<bool> *>) {
    return (alpha.ptr.vb == beta);
  } else if constexpr (std::is_same_v<Data, Ln::Vector<float> *>) {
    return (alpha.ptr.vf == beta);
  } else if constexpr (std::is_same_v<Data, Ln::Matrix<int> *>) {
    return (alpha.ptr.mi == beta);
  } else if constexpr (std::is_same_v<Data, Ln::Matrix<bool> *>) {
...
template <typename Data> inline bool _mmy_delete(Data const &alpha) {
  _mmy_node *omega = _mmy_top;
  _mmy_node *gamma;
  while (omega != nullptr) {
...
inline void _mmy_all() {
  ...
  for (int i = 0; i < Basic_Math::terminal_width; i++) {
    printf("-");
  }
  ...
  while (alpha != nullptr) {
    ...
    if (alpha->data.type == Vi) {
      ...
      printf("type: Vector<float>\n");
    } else if (alpha->data.type == Vb) {
      printf("type: Vector<bool>\n");
    } else if (alpha->data.type == Mi) {
      printf("type: Matrix<int>\n");
...
/*status
Enter: 1.flag
show the status of mode and memory, if the flag is true, then show the memory
detail no return*/
static inline __attribute__((__always_inline__, __used__)) void
status(bool const &alpha) {
  printf("\n");
