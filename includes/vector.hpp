...
private:
  ...
  template <typename T> friend T dot(Vector<T> const &, Vector<T> const &);
  ...
  template <typename T> friend class Linalg::Vector;
...
template <typename Data> Data dot(Vector<Data> const &, Vector<Data> const &);
template <typename op_pls>
