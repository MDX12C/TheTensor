...
Linalg::Matrix<Data> random(Linalg::MaShape const &, Data const &,
                            Data const &);
template <typename Data>
...
private:
  ...
  template <typename T> friend void AddLine_(Matrix<T> &, Vector<T> const &);
  template <typename T> friend void AddRow_(Matrix<T> &, Vector<T> const &);

  friend class Linalg::Vector<Data>;
  template <typename T> friend class Linalg::Matrix;
  ...
  template <typename T>
  friend Matrix<T> Basic_Math::random(Linalg::MaShape const &, T const &,
                                      T const &);
  template <typename T>
...
template <typename T> Matrix<bool> operator==(T const &, Matrix<T> const &);
template <typename T> Matrix<bool> operator!=(T const &, Matrix<T> const &);
template <typename T> Matrix<bool> operator>=(T const &, Matrix<T> const &);
template <typename T> Matrix<bool> operator<=(T const &, Matrix<T> const &);
template <typename T> Matrix<bool> operator>(T const &, Matrix<T> const &);
